...
namespace LicenseAuth
{
	...
	public static long responseTime;
        ...
        public api(string name, string ownerid, string secret, string version)
        {
            ...
            {
                error("Application not setup correctly. Please Check Correctly Or Contact");
                Environment.Exit(0);
            }
        ...
        private static string req(NameValueCollection post_data)
        {
            try
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    var raw_response = client.UploadValues("https://licenseauth.xyz/api/1.2/", post_data);
            ...
            catch (WebException webex)
            {
                ...
                switch (response.StatusCode)
                {
                    case (HttpStatusCode)429: // client hit our rate limit
                        ...
                        Environment.Exit(0);
                        return "";
                    default: // site won't resolve. you should use licenseauth.xyz domain since it's not blocked by any ISPs
